...
const Cursor = () => {
    ...
    //   const cursorSize = 10; // Adjust based on your cursor size
    //   const cursorHalfSize = cursorSize / 2;

    //   // Get the cursor's position but constrain it within the viewport
    //   endX = Math.min(Math.max(e.pageX, cursorHalfSize), width - cursorHalfSize);
    //   endY = Math.min(Math.max(e.pageY, cursorHalfSize), height - cursorHalfSize);

    //   cursorDot.current.style.top = endY + "px";
    //   cursorDot.current.style.left = endX + "px";
    // }
    function positionDot(e) {
      const fullCursorSize = 20; // Update this based on the actual full size of your cursor
      const cursorHalfSize = fullCursorSize / 2;
    
      // Constrain the cursor within the viewport, accounting for its full size
      endX = Math.min(Math.max(e.pageX, cursorHalfSize), width - cursorHalfSize);
      endY = Math.min(Math.max(e.pageY, cursorHalfSize), height - cursorHalfSize);
